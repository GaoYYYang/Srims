...
namespace MIS.Common
{
    ...
    {
        /// <summary> 
        /// 汉字拼音首字母列表 本列表包含了20901个汉字,用于配合 GetChineseSpell 函数使用,本表收录的字符的Unicode编码范围为19968至40869
        /// </summary> 
        private static string[] _StrChineseCharList = 
            { 
        ...
        public static string GetSpell(string strText)
        {
            ...
            foreach (char vChar in strText)
            {
                if ((vChar >= 'a' && vChar <= 'z') || (vChar >= 'A' && vChar <= 'Z') || (vChar >= '0' && vChar <= '9'))
                    myStr.Append(char.ToUpper(vChar, System.Globalization.CultureInfo.CurrentCulture));
                else if ((int)vChar >= 19968 && (int)vChar <= 40869)
                {
                    foreach (string strList in _StrChineseCharList)
                    {
